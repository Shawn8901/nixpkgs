              ...
              case "$kl" in
                ...
                prompt )
                  ...
                  while [[ $success != true ]] && [[ $tries -gt 0 ]]; do
                    ${systemd}/bin/systemd-ask-password ${optionalString cfgZfs.storeEncryptionCredentials("--keyname=zfs-$ds")} --timeout=${toString cfgZfs.passwordTimeout} "Enter key for $ds:" | ${cfgZfs.package}/sbin/zfs load-key "$ds" \
                      && success=true \
                      || tries=$((tries - 1))
                  done
                  [[ $success = true ]]
...

{
  ...
  options = {
    boot.zfs = {
      ...
      storeEncryptionCredentials = mkEnableOption "Stores the encryption credentials in kernel keyring with keyname=zfs-<poolame>";
